...
const COMMENTS_USER_JAMIE = require('../../models/comments-jamie')

router.get('/', async (req, res) => {
    try {
        const comments = await COMMENTS_USER_JAMIE.find()
        res.status(200).json({
            status: 200,
            comments
        });
...
router.post('/', async (req, res) => {
    try {
        const comment = await COMMENTS_USER_JAMIE.create(req.body)
        res.status(200).json({
            status: 200,
            comment
        });
...
router.post('/:id', async (req, res) => {
    try {
        ...
        const commentToUpdate = await COMMENTS_USER_JAMIE.findById(commentId);
...
router.post('/:commentId/like', async (req, res) => {
    try {
        ...
        const comment = await COMMENTS_USER_JAMIE.findById(commentId);
        if (!comment) {
...
router.put('/:id', async (req, res) => {
    try {
        const comment = await COMMENTS_USER_JAMIE.findByIdAndUpdate(req.params.id, req.body, { new: true })
        res.status(200).json({
            status: 200,
            comment
        });
